...
export const populateSearchText = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    console.log(`[Migration] Updated ${updated} games, ${remaining.length} remaining`);
...
export const runFullMigration = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    while (needsMore && runs < 100) { // Safety limit of 100 runs
      const result = await ctx.runMutation(internal.migrations.populateSearchText.populateSearchText, {
        batchSize: 100,
      });
      
      totalUpdated += result.updated;
      needsMore = result.needsMoreRuns;
      runs++;
      
      if (needsMore) {
        console.log(`[Migration] Run ${runs}: Updated ${result.updated}, continuing...`);
      }
    }
    
    console.log(`[Migration] Complete! Total updated: ${totalUpdated} in ${runs} runs`);
