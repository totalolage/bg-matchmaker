...
export const populateAlternateNames = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    const games = await ctx.runQuery(internal.migrations.populateAlternateNames.getGamesNeedingUpdate, {
      cursor: args.cursor,
      limit: batchSize,
    });
    ...
    for (const game of games) {
      try {
        console.log(`[Migration] Fetching data for ${game.name} (BGG ID: ${game.bggId})`);
        
        // Fetch full details from BGG including alternate names
        const details = await BGGDataSource.getGameDetails(ctx, game.bggId);
        
        // Generate searchText
        const searchText = details.alternateNames && details.alternateNames.length > 0
        ...
        console.error(`[Migration] Failed to fetch details for ${game.name}:`, error);
...
export const runMigration = internalAction({
  ...
  handler: async (ctx) => {
    ...
    console.log(`[Migration] Final stats: Processed ${totalProcessed}, Updated ${totalUpdated}, Errors ${totalErrors}`);
